...
class autoCar():
    ...
    def draw(self):
        ...
        # walls
        # for i in range(len(self.x_rec)-1):
        #     temp = list(zip([self.x_rec[i],self.y_rec[i]], [self.x_rec[i+1],self.y_rec[i+1]]))
        #     if self.dir_rec[i] > 0:
...
def tempFunc():
    ...
    for j in range(88):
        ...
        deg_x_axis_next  = deg_x_axis - 10*(math.degrees(math.asin(math.radians(2*math.sin(math.radians(deg_dir))/rad_car))))
        # print('{:5.2f}   {:5.2f}   {:5.2f}'.format(x_next, y_next, deg_x_axis_next))
